...
* If you are using a pre-release build, do not open an issue in [PineappleELEC](https://github.com/351ELEC/351ELEC).  Instead, report the findings in the `#pre-release-feedback` channel on [Discord](https://discord.com/invite/R9Er7hkRMe) (Under "Community Contribution").
...
The latest pre-release build can be found on the **PineappleELEC-pre-release** [releases](https://github.com/351ELEC/351ELEC-pre-release/releases/) page.  Builds are installed as normal (using either a `.tar` or a `.img.gz`).
...
- To find the latest development build, click on the 'green checkbox'(<img src="./images/green-check.png" alt="green checkbox" width="20">) on the PineappleELEC [main](https://github.com/351ELEC/351ELEC) page. Then click *Details*.
...
Moreover, in order to locally build PineappleELEC from sources it's possible to use a docker, more info [here](Contributing-to-351ELEC#docker-usage).
...
### Example:
```
PineappleELEC$ ./scripts/build packages/games/libretro/dosbox-pure
```
...
### Example:
```
PineappleELEC$ ./scripts/install packages/games/libretro/dosbox-pure
```
...
PineappleELEC will have support for a community package repository starting with v1.0.7.  Contributing a package is simple, but there are a few requirements.  Please note that all packages should be submitted as pull requests to the repository.
...
```
git clone https://github.com/351ELEC/351ELEC.git PineappleELEC  
...
### Build only the **ES binary** and copy it to the device
 This might seem obvious, but as emulationstation is just an executable.  You can build it, copy it to the PineappleELEC device and run it **without** making a full PineappleELEC build which saves a lot of time.
  ...
  - **update PineappleELEC `ui/351elec-emulationstation/package.mk`** to point to your clone.
    ...
    - (recommended) if you checked out 351elec-emulationstation into your PineappleELEC folder and use docker to build.  You can build as follows.  **NOTE** that removing the source is needed to ensure the git source is properly re-copied:
    ```
...
The above method is recommended due to difficulty setting up all proper files to simulate a PineappleELEC device - but building ES on x86_64 may have interesting/novel uses as you can run the PineappleELEC ES on a x86_64 PC/VM for testing.  **Requires Linux - tested on Ubuntu 20.04**
...
- **Setup PineappleELEC files**
  - it may be possible to have emulation station look for files in your home directory, but I try and make it match PineappleELEC and set home to /storage
    - `export HOME=/storage`
    - NOTE: You'll need to set `HOME` everytime you open a new terminal.  It's also possible to not set home to /storage and most files will come from the users home directory, but some things in PineappleELEC are hard coded to /home.
  - Ensure you have PineappleELEC checked out next to 351elec-emulationstation (git clone https://github.com/351ELEC/351ELEC/ ../351ELEC)
